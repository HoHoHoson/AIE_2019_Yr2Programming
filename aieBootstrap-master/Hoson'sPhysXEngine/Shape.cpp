...
{
	m_VerticeCount = vertices < 3 ? 0 : vertices;
	m_Radius = radius;
	m_Colour = colour;
}
...
{
	float nth = (n % m_VerticeCount + m_VerticeCount) % m_VerticeCount;
	assert(nth >= 0 && nth < m_VerticeCount);

	// May replace the "0" with the objects rotation in radians if I get to it
	float x = m_Radius * cosf((2 * M_PI) * (nth / m_VerticeCount) + 0) + m_Position.x;
	float y = m_Radius * sinf((2 * M_PI) * (nth / m_VerticeCount) + 0) + m_Position.y;
